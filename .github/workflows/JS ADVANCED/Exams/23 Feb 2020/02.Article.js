class Article {
    ...
    get likes() {
        let message = '';
        this._likes.length === 0 ? message = `${this.title} has 0 likes` : 'nothing';
        this._likes.length === 1 ? message = `${this._likes[0]} likes this article!` : 'nothing';
        this._likes.length > 1 ? message = `${this._likes[0]} and ${this._likes.length - 1} others like this article!` : 'nothing';
        return message;
    ...
    dislike(username) {
        ...
        this._likes.filter(usernames => usernames !== username)
        return `${username} disliked ${this.title}`
    }

    comment(username, content, id) {

        if (this._comments.some((comment) => comment.id === id) || id == undefined) {
            let newId;
